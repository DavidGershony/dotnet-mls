...
public class HkdfTests
{
    ...
    // ----------------------------------------------------------------
    // RFC 5869 Appendix A test vectors — authoritative correctness proof.
    // If these pass, our managed HKDF-Expand is bit-for-bit correct.
    // ----------------------------------------------------------------
    ...
    public void Rfc5869_TestCase1_Sha256_BasicTest()
    {
        ...
        var hkdf = new HkdfProvider();
        Assert.Equal(expectedPrk, hkdf.Extract(salt, ikm));
        Assert.Equal(expectedOkm, hkdf.Expand(expectedPrk, info, 42));
    }

    [Fact]
    [Trait("Category", "Hkdf")]
    public void Rfc5869_TestCase2_Sha256_LongInputsAndOutput()
    {
        // RFC 5869 Appendix A.2 — exercises multiple HMAC iterations (L=82, hashLen=32, n=3).
        ...
        var hkdf = new HkdfProvider();
        Assert.Equal(expectedPrk, hkdf.Extract(salt, ikm));
        Assert.Equal(expectedOkm, hkdf.Expand(expectedPrk, info, 82));
    }

    [Fact]
    [Trait("Category", "Hkdf")]
    public void Rfc5869_TestCase3_Sha256_EmptySaltAndInfo()
    {
    ...
    [Fact]
    [Trait("Category", "Hkdf")]
    public void ExpandSha256_StaticConvenience_MatchesInstance()
    {
        byte[] prk = Convert.FromHexString("077709362c2e32df0ddc3f0dc47bba6390b6c73bb50f9c3122ec844ad7c2b3e5");
        byte[] info = Convert.FromHexString("f0f1f2f3f4f5f6f7f8f9");

        byte[] viaInstance = new HkdfProvider().Expand(prk, info, 42);
        byte[] viaStatic = HkdfProvider.ExpandSha256(prk, info, 42);

        Assert.Equal(viaInstance, viaStatic);
    }
}
