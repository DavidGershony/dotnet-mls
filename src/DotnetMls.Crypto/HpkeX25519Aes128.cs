...
///   (KEM ID = 0x0020, KDF ID = 0x0001, AEAD ID = 0x0001)
///
/// Operates in Base mode (mode = 0x00) only.
/// </summary>
public sealed class HpkeX25519Aes128 : IHpke
{
    // RFC 9180 constants
    private const ushort KemId = 0x0020;   // DHKEM(X25519, HKDF-SHA256)
    private const ushort KdfId = 0x0001;   // HKDF-SHA256
    private const ushort AeadId = 0x0001;  // AES-128-GCM
    private const byte ModeBase = 0x00;

    private const int Nk = 16;   // AES-128-GCM key size
    private const int Nn = 12;   // AES-128-GCM nonce size
    ...
    /// <summary>
    /// Creates a new HPKE instance using the provided cryptographic providers.
    /// </summary>
    public HpkeX25519Aes128(X25519Provider x25519, HkdfProvider hkdf, AesGcmProvider aead)
    {
        _x25519 = x25519;
        _hkdf = hkdf;
        _aead = aead;
    ...
    /// <summary>
    /// Creates a new HPKE instance with default providers.
    /// </summary>
    public HpkeX25519Aes128()
        : this(new X25519Provider(), new HkdfProvider(), new AesGcmProvider())
    {
    }
    ...
    public byte[] Seal(byte[] recipientPublicKey, byte[] info, byte[] aad, byte[] plaintext)
    {
        ...
        // Derive key and nonce from shared secret via KeySchedule
        var (key, baseNonce) = KeyScheduleS(sharedSecret, info);

        // Encrypt
        var ct = _aead.Encrypt(key, baseNonce, aad, plaintext);

        // Return enc || ct (kem_output prepended to ciphertext)
    ...
    // ---- KEM internal methods ----

    /// <summary>
    /// ExtractAndExpand per RFC 9180 Section 4.1:
    ///   eae_prk = LabeledExtract("", "eae_prk", dh)  [using KEM suite_id]
    ...
    /// <summary>
    /// LabeledExtract for KEM operations (uses KEM suite_id).
    /// labeled_ikm = "HPKE-v1" || suite_id || label || ikm
    /// return Extract(salt, labeled_ikm)
    /// </summary>
    private byte[] LabeledExtractKem(byte[] salt, byte[] label, byte[] ikm)
    {
        var labeledIkm = BuildLabeledIkm(KemSuiteId, label, ikm);
        return _hkdf.Extract(salt, labeledIkm);
    }

    /// <summary>
    /// LabeledExpand for KEM operations (uses KEM suite_id).
    /// labeled_info = I2OSP(L, 2) || "HPKE-v1" || suite_id || label || info
    /// return Expand(prk, labeled_info, L)
    /// </summary>
    private byte[] LabeledExpandKem(byte[] prk, byte[] label, byte[] info, int length)
    {
        var labeledInfo = BuildLabeledInfo(KemSuiteId, label, info, length);
        return _hkdf.Expand(prk, labeledInfo, length);
    }

    // ---- HPKE KeySchedule (Base mode) ----

    /// <summary>
    /// Simplified KeySchedule for Base mode (mode = 0x00, no PSK).
    /// Returns (key, base_nonce) for AEAD operations.
    /// </summary>
    ...
    /// <summary>
    /// LabeledExtract for HPKE operations (uses HPKE suite_id).
    /// </summary>
    private byte[] LabeledExtractHpke(byte[] salt, byte[] label, byte[] ikm)
    {
        var labeledIkm = BuildLabeledIkm(HpkeSuiteId, label, ikm);
        return _hkdf.Extract(salt, labeledIkm);
    }

    /// <summary>
    /// LabeledExpand for HPKE operations (uses HPKE suite_id).
    /// </summary>
    private byte[] LabeledExpandHpke(byte[] prk, byte[] label, byte[] info, int length)
    {
        var labeledInfo = BuildLabeledInfo(HpkeSuiteId, label, info, length);
        return _hkdf.Expand(prk, labeledInfo, length);
    }
    ...
    /// <summary>
    /// Builds labeled_ikm = "HPKE-v1" || suite_id || label || ikm
    /// </summary>
    private static byte[] BuildLabeledIkm(byte[] suiteId, byte[] label, byte[] ikm)
    {
        ...
        Buffer.BlockCopy(HpkeV1Prefix, 0, result, offset, HpkeV1Prefix.Length);
        offset += HpkeV1Prefix.Length;

        Buffer.BlockCopy(suiteId, 0, result, offset, suiteId.Length);
        offset += suiteId.Length;

        Buffer.BlockCopy(label, 0, result, offset, label.Length);
        offset += label.Length;
    ...
    /// <summary>
    /// Builds labeled_info = I2OSP(L, 2) || "HPKE-v1" || suite_id || label || info
    /// </summary>
    private static byte[] BuildLabeledInfo(byte[] suiteId, byte[] label, byte[] info, int length)
    {
        ...
        // I2OSP(length, 2) - big-endian uint16
    ...
    private static byte[] BuildHpkeSuiteId()
    {
        ...
        result[offset++] = (byte)(KemId >> 8);
        result[offset++] = (byte)(KemId & 0xFF);
        result[offset++] = (byte)(KdfId >> 8);
        result[offset++] = (byte)(KdfId & 0xFF);
        result[offset++] = (byte)(AeadId >> 8);
        result[offset] = (byte)(AeadId & 0xFF);
